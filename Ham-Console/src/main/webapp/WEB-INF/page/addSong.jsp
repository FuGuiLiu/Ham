    ...
    <script>
        ...
        function submitFile() {
            ...
            $("#song-form").ajaxSubmit({
                url: "/console/song/fileUpload.go",
                data: {
                    fileType: "mp3file"
                },
                dataType: "json",
...
<div class="wrapper">
    ...
    <div class="page-content">
        <div class="content container">
            <div class="row">
                <div class="col-lg-12">
                    <h2 class="page-title">${isModify!=null?"修改音乐":"添加音乐"}<small> 上架新歌</small></h2>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12">
                        ...
                        <div class="widget-content">
                            <div class="body">
                                <form data-validate="parsley" method="post"
                                      action="${isModify!=null?'/console/song/modify.go':"/console/song/addSong.go"}"
                                      novalidate=""
                                      class="form-horizontal label-left" id="song-form">

                                    <fieldset>
                                        <input type="hidden" name="SID" value="${song.SID!=null?song.SID:""}">
                                        ...
                                        <div class="control-group">
                                            <label for="srid" class="control-label">歌手<span
                                                    class="required">*</span></label>
                                            <div class="controls form-group">
                                                <div data-toggle="buttons" class="btn-group col-sm-2">
                                                    <select id="srid" name="SRID" class="form-control ">
                                                        <c:forEach items="${singers}" var="songers" varStatus="status">
                                                            <option value="${songers.SRID}" ${song.SRID==song.SRID?"selected='selected'":""}>${songers.SRNAME}</option>
                                                        </c:forEach>
                                                    </select>
                                        ...
                                        <div class="control-group">
                                            <label for="aid" class="control-label">专辑<span
                                                    class="required">*</span></label>
                                            <div class="controls form-group">
                                                <div data-toggle="buttons" class="btn-group col-sm-2">
                                                    <select id="aid" name="AID" class="form-control ">
                                                        <c:forEach items="${albums}" var="album">
                                                            <option value="${album.AID}" ${album.AID==song.AID?"selected='selected'":""}>${album.ANAME}</option>
                                                        </c:forEach>
                                                    </select>
                                        ...
                                        <div class="control-group">
                                            <label for="tid" class="control-label">流派<span
                                                    class="required">*</span></label>
                                            <div class="controls form-group">
                                                <div data-toggle="buttons" class="btn-group col-sm-2">
                                                    <select id="tid" name="TID" class="form-control ">
                                                        <c:forEach items="${mTypes}" var="mtype">
                                                            <option value="${mtype.TID}" ${mtype.TID==song.TID?"selected='selected'":""}>${mtype.TNAME}</option>
                                                        </c:forEach>
                                                    </select>
                                                </div>
                                            </div>
                                        ...
                                        <div class="control-group">
                                            ...
                                            <div class="controls form-group">
                                                <div data-toggle="buttons" class="btn-group col-sm-2">
                                                    <select id="isNew" name="IS_NEW" class="form-control ">
                                                        <option value="1" ${song.IS_NEW==1?"selected='selected'":""} >
                                                            是
                                                        </option>
                                                        <option value="0" ${song.IS_NEW==0?"selected='selected'":""}>
                                                            否
                                                        </option>
                                                    </select>
                                            ...
                                            <div class="controls form-group">
                                                <div data-toggle="buttons" class="btn-group col-sm-2">
                                                    <select id="isHot" name="IS_HOT" class="form-control ">
                                                        <option value="1" ${song.IS_HOT==1?"selected='selected'":""}>
                                                            是
                                                        </option>
                                                        <option value="0" ${song.IS_HOT==0?"selected='selected'":""}>
                                                            否
                                                        </option>
                                                    </select>
                                    ...
                                    <fieldset>
                                        ...
                                        <div class="control-group">
                                            ...
                                            <div class="controls form-group">
                                                <textarea class="form-control" name="LRC" rows="3"
                                                          id="description">${song.LRC}</textarea>
                                                <span class="help-block">请入lrc格式歌词</span></div>
                                        </div>
                                        <div class="control-group">
                                            <label for="i1-file" class="control-label">选择歌曲 <span
                                                    class="required">*</span></label>
                                            <!--<div class="col-sm-4 control-label">选择文件</div>-->
                                            <div class="col-sm-6">
                                                <div class="input-group">
                                                    <input id='mp3loc' class="form-control"
                                                           onclick="$('#i-file').click();">
                                                    <label class="input-group-btn">
                                                        <input type="button" id="i1-check" value="选择歌曲文件"
                                                               class="btn btn-primary" onclick="$('#i1-file').click();">
                                                    </label>
                                                </div>
                                            </div>
                                            <input type="hidden" id="mp3" name="MP3" lay-verify="pic"
                                                   value="${song.MP3!=null?song.MP3:""}">
                                            <input type="hidden" id="lastMp3" name="lastMp3">
                                            <input type="file" name="mp3file" id='i1-file' accept=".mp3, .wma,.flac"
                                                   onchange="submitFile()" style="display: none">
                                        </div>
                                    </fieldset>
                                    <div class="form-actions text-right">
                                        <button class="btn btn-primary" type="submit" ${isModify!=null?"更新歌曲":"提交歌曲"}>
                                            提交歌曲
                                        </button>
                                    </div>
                                </form>
